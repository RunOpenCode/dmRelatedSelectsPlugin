...
(function($){
	...
	var RelatedSelect = function(context, opts) {
		...
		function populate($caller,$select,o){
			var params = {}, highestIndex = -1;
            
			// take those selectors and serialize the data in them
            // seek highest index as well...
            // this is of course very stupid, but we do not know when we will find highest index
            $.each(selects, function(){
                var $currentSelect = $context.find('select[name="' + this.toString() + '"]');                
                if ($currentSelect.val()) {
                    params[this] = {
                        val: $currentSelect.val(),
                        metadata: $currentSelect.metadata()
                    };
                    highestIndex = Math.max($currentSelect.metadata().index, highestIndex);
                };
            });
            
            // Now, we are adding the last child according to the highest index            
            highestIndex++;
            // This is the one that we are looking for
            $.each(selects, function(){
                var $currentSelect = $context.find('select[name="' + this.toString() + '"]');                  
                if ($currentSelect.prop('name') && $currentSelect.metadata().index == highestIndex) {
                    params[this] = {                        
                        metadata: $currentSelect.metadata()
                    };
                };
